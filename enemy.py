...
class Enemy:
	def __init__(self):
		...
		# Colors
		self.cloak_color = "#363636" # Near black
		...
		self.projectile_color = "#ffa500" # Orange
	...
	def update(self, enemy):
		...
		if self.going_up and self.pos.y > 0:
			self.pos.y -= self.movement_speed
		elif self.pos.y < 400:
			self.going_up = False
			self.pos.y += self.movement_speed
		else:
			# Leave it to the next frame
			self.going_up = True
		if self.shoot_interval < 1:
			self.shoot()
